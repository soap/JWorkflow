...
class WFInstallerPlugin extends JAdapterInstance
{
	...
	{
		$results = array();
		$folder_list = JFolder::folders(WFPATH_PLUGINS);

		foreach ($folder_list as $folder)
		{
			$file_list = JFolder::files(WFPATH_PLUGINS . '/' . $folder, '\.xml$');
			foreach ($file_list as $file)
			{
				$manifest_details = JApplicationHelper::parseXMLInstallFile(WFPATH_PLUGINS . '/plugins/' . $folder . '/' . $file);
			...
			$folder_list = JFolder::folders(WFPATH_PLUGINS . '/plugins/' . $folder);
			foreach ($folder_list as $plugin_folder)
			{
				$file_list = JFolder::files(WFPATH_PLUGINS . '/plugins/' . $folder . '/' . $plugin_folder, '\.xml$');
				foreach ($file_list as $file)
				{
					$manifest_details = JApplicationHelper::parseXMLInstallFile(
						WFPATH_PLUGINS . '/plugins/' . $folder . '/' . $plugin_folder . '/' . $file
					);
					$file = JFile::stripExt($file);
