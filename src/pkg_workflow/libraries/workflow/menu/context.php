...
class WFMenuContext
{
    ...
    public function itemEdit($asset, $id = 0, $access = false, $return_page=false)
    {
        if (!$access) return '';
        
        $return = '';
        if ($return_page === true ) {
        	$return = '&return='.base64_encode(JFactory::getURI()->toString());	
        }elseif (strlen($return_page)) {
        	$return = '&return='.base64_encode($return_page);	
        }
        
        $icon   = 'icon-pencil';
        $action = JRoute::_('index.php?option=' . $this->component . '&task=' . strval($asset) . '.edit&id=' . intval($id).$return);
