...
?>
<form action="<?php echo JRoute::_('index.php?option=com_workflow&view=bindings');?>" method="post" name="adminForm">
	<fieldset id="filter-bar">
		<div class="filter-search fltlft">
			...
			<input type="text" name="filter_search" id="filter_search"
				...
				title="<?php echo JText::_('COM_WORKFLOW_BINDINGS_FILTER_SEARCH_DESC'); ?>" />
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th>
					<?php echo JHtml::_('grid.sort', 'COM_WORKFLOW_HEADING_CONTEXT', 'a.context', $listDirn, $listOrder); ?>
				</th>	
				<th>
					<?php echo JText::_('COM_WORKFLOW_HEADING_MORE_MAPPINGS')?>
				</th>				
				<th>
					<?php echo JHtml::_('grid.sort', 'JGLOBAL_TITLE', 'workflow_title', $listDirn, $listOrder); ?>
				</th>			
				<th width="5%">
				...
				<th width="10%" class="nowrap">
					...
					<?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'bindings.saveorder'); ?>
				</th>
				<th width="10%">
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					<?php if ($item->checked_out) : ?>
						<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'bindings.', $canCheckin); ?>
					<?php endif; ?>
					<?php if ($canCreate || $canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_workflow&task=binding.edit&id='.$item->id);?>">
						<?php echo $this->escape($item->context); ?></a>
					<?php else : ?>
				...
				<td class="order">
					<?php if ($canChange) : ?>
						<span><?php echo $this->pagination->orderUpIcon($i,
							...
							'bindings.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
						<span><?php echo $this->pagination->orderDownIcon($i,
							$this->pagination->total,
							($item->workflow_id == @$this->items[$i+1]->workflow_id),
							'bindings.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
