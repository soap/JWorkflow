...
/**
 * Tringger Instances model.
 *
 * @package     JWorkflow
 * @subpackage  com_workflow
 * @since       1.0
 */
class WorkflowModelTriggerinstances extends JModelList
{
	/**
	...
	protected function getListQuery()
	{
		...
		$query->from('#__wf_trigger_instances AS a');
		...
		if (is_numeric($transitionId) && ($transitionId != 0 || !empty($transitionId) )) {
		...
		if (count($items)) {
			for ($i = 0; $i <count($items); $i++) {
				...
				for($j=0; $j < count($triggers); $j++) {
					if (!is_object($triggers[$j])) continue;
