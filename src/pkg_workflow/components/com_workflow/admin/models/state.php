 ...
 * @since       1.0
 */
class WorkflowModelState extends JModelAdmin
{
	/**
	 * Method to get the State form.
	 *
	 ...
	 * @since   1.0
	 */
	public function getTable($type = 'State', $prefix = 'WorkflowTable', $config = array())
	{
	...
	protected function loadFormData()
	{
		...
		if (empty($data->id)) {
			if (empty($data->workflow_id))	{
				...
				$value = $app->getUserStateFromRequest('com_workflow.filter.workflow_id', 'filter_workflow_id');

				if (!empty($value)) $data->workflow_id = $value;
			}
		}
		return $data;
	}
