 ...
 * @since       1.0
 */
class WorkflowModelRole extends JModelAdmin
{
	/**
	 * Method to get the Role form.
	 *
	 ...
	 * @since   1.0
	 */
	public function getTable($type = 'Role', $prefix = 'WorkflowTable', $config = array())
	{
	...
	protected function preprocessForm(JForm $form, $data, $group='content')
	{
		...
		if ( !($user->authorise('core.edit.state', 'com_workflow')) ) {
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				...
				$query->from('#__wf_user_roles');
				$query->where('method = '.(int) $table->method);
