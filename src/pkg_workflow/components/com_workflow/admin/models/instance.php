...
class WorkflowModelInstance extends JModelItem 
{
	...
	public function makeTransition($data)
	{
		...
		$this->_from_state_id = $instance->workflow_state_id; 
	...
	public function getTransitionLogMessage($context, $item_id, $transition_id, $from_state_id=null) 
	{
		...
		if (empty($from_state_id) && !empty($this->_from_state_id) ) {
			$from_state_id = $this->_from_state_id;
		}
		
		if (!empty($from_state_id)) $query->where('from_state_id ='.(int)$from_state_id);
